...
const getaProduct = asyncHandle(async (req, res) => {
  const { id } = req.params
  try {
    const findProduct = await ProductModel.findById(id).populate('sizes')
    res.status(200).send({
      success: true,
...
module.exports = {
  ...
  , updateProduct, deleteProduct, addToWishlist, rating, getSlugProduct, searchProductController
}
